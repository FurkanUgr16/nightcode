...
export function useChat(sessionId: string, initialMessages: Message[]) {
  ...
  const emitParts = useCallback(
    (requestId: string, parts: ClientMessagePart[]) => {
      ...
      const activeStram = activeStreamRef.current

      if (!activeStram) return

      activeStram.parts = snapshot
      setStreaming({
        status: 'streaming',
        parts: snapshot,
        mode: activeStram.mode,
        model: activeStram.model,
      })
    },
    [isActiveRequest],
  )
  ...
  const runStream = useCallback(
    async ({ mode, model, request }: RunStreamParams) => {
      ...
      try {
        ...
        await handelStream(response, activeStream)
      } catch (error) {
    ...
    [handelStream, isActiveRequest, updateMessages, clearStream],
  )
